...
public interface CardRepository extends JpaRepository<Card, String> {
	...
	@Query(nativeQuery = true, value = "SELECT * FROM card LEFT JOIN card_rooms ON card.id = card_rooms.card_id WHERE card_type = ?1 AND (rooms != ?2 OR rooms IS NULL ) ORDER BY RANDOM() LIMIT 1")
